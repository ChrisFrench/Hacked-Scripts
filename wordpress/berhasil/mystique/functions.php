...
if (WP_VERSION < 2.6): // disable theme front end if wp < 2.6
  ...
  <div class='error fade'>
   <p>
   <?php
    printf(__('Your site is running on %1$s. Mystique requires at least %2$s.','mystique'), 'Wordpress '.WP_VERSION, '<a href="http://codex.wordpress.org/Upgrading_WordPress">Wordpress 2.7</a>');           if (current_user_can('switch_themes') && !is_admin()) echo '<br /><a href="'.get_bloginfo('wpurl').'/wp-admin/">'.__("(Dashboard)","mystique").'</a>';
   ?>
   </p>
  </div>
  <?php if(!is_admin()) die();
